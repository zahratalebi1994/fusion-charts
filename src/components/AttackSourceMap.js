...
function AttackSourceMap (props) {
...
function countAttack (attacks, source) {
    const countTypes = attacks.filter(attacks => attacks.source_name === source.toString());
...
function prepareChartData(attacks) {
    ...
    attacks.map( attack => {
        data.push({
            id: findProvinceID(attack.source_name),
            value: countAttack(attacks, attack.source_name),
            showLabel: "1"
        })
    });
    //remove duplicates
